...
function show(nseconds) {
  ...
  editor.setValue(frame.getText(), 1);
  if (frame.selectionStartLine == frame.selectionEndLine && frame.selectionStartColumn == frame.selectionEndColumn) {
    editor.moveCursorTo(frame.selectionStartLine - 1, frame.selectionStartColumn - 1);
  } else {
    editor.getSelection().setSelectionRange(new Range(frame.selectionStartLine - 1, frame.selectionStartColumn - 1, frame.selectionEndLine - 1, frame.selectionEndColumn));
  }
  editor.focus();
}
