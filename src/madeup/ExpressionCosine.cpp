...
Co<Expression> ExpressionCosine::evaluate(Environment &env) const {
  ...
  if (degrees_closure.IsNull()) {
    throw UnlocatedException("Function cos expects a value named degrees. No value named degrees is defined.");
  }
  ...
  throw UnlocatedException("Function cos expects a number. " + degrees_closure->getSource() + " is not a number.");
}
